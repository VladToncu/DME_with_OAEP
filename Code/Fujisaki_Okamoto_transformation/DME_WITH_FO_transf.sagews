#Importing libraries for padding
...
#The parameters used for generate the OAEP padding.
nBits = e*m*n;
k0BitsInt = 128;
k1BitsInt = 0;
encoding='utf-8'
...
#Function to transform a string into elements of the finite field
def transf_String_to_Finite_SK(message):
    message_to_encrypt = matrix(K,12,1)
    vec = []
    for j in range(0,12):
        vec=[]
        for i in range(0,e):
    ...
    for i in range(0,m):
        ...
        for j in range(0,m):
                if(A[i][j]!=0):
                    ...
                    current_pol_id = 0
                    for l in range(0,n):
                        current_pol = current_pol + (x[j*n+l][0]^A[i][j].lift())*(gens[counterPol]^(n-1-l))
                    poly_G1 = poly_G1 + [current_pol]
                    for l in range(0,n):
                        current_pol_id = current_pol_id + (gens[counterPol]^(n-1-l))
                    idPol = idPol * current_pol_id
                    counterPol = counterPol + 1
                    if(counterPol == s):
                        break;
        for k in range(0,s):
            pol = pol * poly_G1[k]
        if(len(pol.coefficients()) == n^s):
            row_results[i]=pol.coefficients()
        else:
            sofar = 0
            for k in range(0,n^s):
                if(sofar > len(pol.monomials())-1):
                    row_results[i,k] = 0
                elif(idPol.monomials()[k] == pol.monomials()[sofar]):
                    row_results[i,k] = pol.coefficients()[sofar]
                    sofar += 1
                else:
                    row_results[i,k] = 0
...
#Initialise a random message that we want to encrypt
message = SystemRandom().getrandbits(k0BitsInt)
message = format(message, '0128b')

#This is the encryption of the Fujisaki-Okamoto transformation

#Create G and H' for Fujisaki-Okamoto
oracle1 = hashlib.sha256()
oracle3 = hashlib.sha256()

#Generate random string sigma
randString = SystemRandom().getrandbits(k0BitsInt)
randString = format(randString, '0128b')

#Compute G(sigma)
oracle1.update(randString)
result_oracle1 = str(bin(int(oracle1.hexdigest(), 16))[2:])
if(len(result_oracle1)<256):
    result_oracle1 += '0'*(256-len(result_oracle1))

#Compute H'(sigma)
oracle3.update(randString)
result_oracle3 = str(bin(int(oracle3.hexdigest(), 16))[2:])
if(len(result_oracle3)<256):
    result_oracle3 += '0'*(256-len(result_oracle3))

#Run AES-256 (in Sagemath it is known as RijndaelGF) - this is the symmetric encryption of the Fujisaki-Okamoto transformation
rgf = RijndaelGF(4, 8)

enc_sym = rgf.encrypt(message, result_oracle1, format="binary")

#Generate H(sigma,enc_sym)
pad_R = hmac.new(enc_sym,digestmod=hashlib.sha256)
pad_R.update(randString)
pad_R_oracle = str(bin(int(pad_R.hexdigest(), 16))[2:])

#Generate the asymmetric encryption of the Fujisaki-Okamoto transformation
to_enc_pub = randString + pad_R_oracle

to_enc_pub = int(to_enc_pub[::-1])
to_enc_pub_elem = transf_String_to_Finite(to_enc_pub)

enc_pub = encrypt_with_public_key(to_enc_pub_elem)

#The hybrid encryption returns (c1,c2,c3), where c1=asymm encryption, c2=symm encryption and c3=H'(sigma)
FO_enc = (enc_pub,enc_sym,result_oracle3)


#This is the decryption of the Fujisaki-Okamoto transformation

#Get the 3 elements of the tuple that we receive from the hybrid encryption
c1 = FO_enc[0]
c2 = FO_enc[1]
c3 = FO_enc[2]

#Run the decryption algorithm of the DME (asymmetric encryption)
dec_pub = decrypt_with_secret_key(c1)
dec_string = ""
for i in range(0,m*n):
    dec_string = transf_finite_to_String(dec_pub[i][0]) + dec_string
dec_string = dec_string[::-1]

#Get sigma' from the result of the decryption
rand_str = dec_string[0:128]

#Generate G(sigma')
oracle1 = hashlib.sha256()
oracle1.update(rand_str)
result_oracle1 = str(bin(int(oracle1.hexdigest(), 16))[2:])
if(len(result_oracle1)<256):
    result_oracle1 += '0'*(256-len(result_oracle1))
...
#Generate H'(sigma')
oracle3 = hashlib.sha256()
oracle3.update(rand_str)
result_oracle3 = str(bin(int(oracle3.hexdigest(), 16))[2:])
if(len(result_oracle3)<256):
    result_oracle3 += '0'*(256-len(result_oracle3))

#Decrypt the message from the AES encryption using G(sigma')
mess = rgf.decrypt(c2, result_oracle1, format="binary")

#Generate H(sigma',c2)
res_pad_R = hmac.new(c2,digestmod=hashlib.sha256)
res_pad_R.update(rand_str)
res_pad_R_oracle = str(bin(int(res_pad_R.hexdigest(), 16))[2:])

#Generate the public key encryption again to verify whether it is equal to c1
verif_string = rand_str + res_pad_R_oracle
verif_string = int(verif_string[::-1])

#Checking whether c1 is the same as DME(sigma'+H(sigma',c2)) and whether c3 == H'(sigma')
if(c1 == encrypt_with_public_key(transf_String_to_Finite(verif_string)) and c3 == result_oracle3):
    print "\nDecrypted message is: "
    print mess
    if(mess == message):
        print "\nThe original input is equal with the decryption output"
else:
    print "Fail"
