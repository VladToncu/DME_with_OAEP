︠9e945c47-85a7-456a-91db-f3506b27acdd︠
...
Kpol = PolynomialRing(K,'xtest',64).quotient(p)
c = Kpol.gens()
...
for i in range(0,m):
    for j in range(0,mon):
        row[i,0] += F_generated_1[i,j]*c[j]


#Generating the polynomial system that is send to the F4 algorithm
for i in range(0,m):
...
GBaux = GB
mydict = []
#Replace the variables with the ones in the monomial vector to verify whether the Grobner basis is valid and provides result=0 after we substitute every variable with its value
for i in range(0,mon):
    mydict.append({c[i]:monoms_for_pt[i][0]})
for i in range(0,mon):
    GB = GB.subs(mydict[i])
print GB
...
for i in range(0,m):
    for j in range(0,mon):
        row[i,0] += F_generated_2[i,j]*c[j]


#Generating the polynomial system that is send to the F4 algorithm
for i in range(0,m):
    row[i,0] = row[i,0] - Kpol(encrypt_pub[i+m,0])
...
#Replace the variables with the ones in the monomial vector to verify whether the Grobner basis is valid and provides result=0 after we substitute every variable with its value
for i in range(0,mon):
    mydict.append({c[i]:monoms_for_pt[i+mon][0]})
for i in range(0,mon):
    GB = GB.subs(mydict[i])
print GB
