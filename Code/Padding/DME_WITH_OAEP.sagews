...
def decrypt_with_secret_key(x):
    ...
    #After we decrypt, we have to unpad the message we receive and return the original message
    dec_message=""
    for i in range(0,m*n):
        dec_message = transf_finite_to_String(decrypted[i][0]) + dec_message
    dec_message = dec_message[::-1]
    out = unpad(dec_message)
    return text_from_bits(out)
...
def encrypt_with_public_key(x):
    message_to_encrypt = matrix(K,m*n,1)
    #This pads the message before encrypting the message
    output_string = pad(x)
    print len(output_string)
    output = int(output_string[::-1])

    x = transf_String_to_Finite(output)
...
def pad(msg):
    ...
    #Padding the message with k1 zeros
    zeroPaddedMsg = binMsg
    if len(str(binMsg)) <= (nBits-k0BitsInt-k1BitsInt):
...
def unpad(msg):
    ...
    #Retrieving the message+the k1 zeros
    oracle1.update(r)
    result_oracle1 = str(bin(int(oracle1.hexdigest(), 16))[2:])
    len_oracle1 = len(result_oracle1)
    result_oracle1 = '0'*(256-len_oracle1) + result_oracle1
    msgWith0s = xor(x,result_oracle1)
    padded0s = msgWith0s[-k1BitsInt:]
    if(padded0s != ('0'*k1BitsInt)):
        return ('0'*(nBits-k0BitsInt-k1BitsInt))
    return msgWith0s[0:(nBits-k0BitsInt-k1BitsInt)]

#Performing the DME encryption and decryption to see whether the original message is the same with what we decrypt
test = "testme"

encrypt_pub = encrypt_with_public_key(test)
decrypt_pub = decrypt_with_secret_key(encrypt_pub)
